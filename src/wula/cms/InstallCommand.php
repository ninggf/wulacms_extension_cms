...
class InstallCommand extends ArtisanCommand {
    ...
    protected function getOpts() {
        return ['c::file' => 'install cms from config file'];
    }
    ...
    protected function execute($options) {
        ...
        $dashboard = 'backend';
        $domain    = '';
        if (isset($options['c']) || $params) {
            if (isset($options['c'])) {
                $opts = parse_ini_file(APPROOT . $options['c']);
            } else {
            ...
            if ($opts) {
                ...
                $dbcharset = aryget('charset', $opts, 'UTF8MB4');
                $username  = aryget('username', $opts, 'admin');
                $password  = aryget('password', $opts, rand_str(12));
